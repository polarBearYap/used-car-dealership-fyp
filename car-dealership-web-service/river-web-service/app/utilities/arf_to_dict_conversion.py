...
def extract_rg_arf(arf_model, feature_names):
    ...
    for hoeffding_tree_member in arf_model:
        ...
        while len(queue) > 0:
            ...
            if value == 0:
                value = 1
            # Scale down the prediction value by the total number of base learners
            hoeffding_dict["values"][node_idx] = value * scaling


            # Retrieve and assign the weighted count of samples at the current node
            hoeffding_dict["node_sample_weight"][node_idx] = node.stats.mean.n
