...
def test_arf_cf(arf_cf, X, Y):
    ...
    train_f1_score_river = []
    train_precision_river = []
    train_recall_river = []
    train_roc_auc_river = []

    length = len(X)
    desc = f'Training adaptive random forest algorithm'
    with tqdm(total = length, position=0, leave=True, desc=desc) as pbar:
        progress_unit = 1
        for x, y in zip(X.to_numpy(), Y):
            ...
            if  data_count != 0 and data_count % 100 == 0:
                ...
                train_f1_score_river.append(f1_metric.get())
                train_precision_river.append(precision_metric.get())
                train_recall_river.append(recall_metric.get())
                train_roc_auc_river.append(roc_auc_metric.get())
            data_count += 1
